void HTTP_init(void) {
  ...
  HTTP.on("/config.json", handle_ConfigJSON); // Формирование config.json страницы для передачи данных в web интерфейс
  ...
  httpUpdater.setup(&HTTP);       // Добавляем функцию Update для перезаписи прошивки по WiFi при 1М(256K SPIFFS) и выше
...
void handle_Set_Ssidap() { 
  // jsonWrite(configSetup, "ssidAP", HTTP.arg("ssidAP"));         // запись по новому
...
void handle_ConfigJSON() {
  ...
  // json["ip"] = WiFi.localIP().toString();  // не записывается ??
  json["time"] = GetTime();
  json["date"] = GetDate();
...
void handle_add_DHT() {
  ...
  String temperature = (String)HTTP.arg("tm");
  String humidity = (String)HTTP.arg("ph");
  String ssure = (String)HTTP.arg("ss");
  String phdht = (String)HTTP.arg("phDHT");
      SoketData ("tmHaus", temperature, jsonRead(jsonData,"tmHaus"));
      SoketData ("phHaus", humidity, jsonRead(jsonData,"phHaus"));
      SoketData ("ssHaus", ssure, jsonRead(jsonData,"ssHaus"));
      SoketData ("phDHT", phdht, jsonRead(jsonData,"phDHT"));
  jsonWrite(jsonData, "tmHaus", temperature);          // отправить температуру в jsonData
  jsonWrite(jsonData, "phHaus", humidity);             // отправить влажность в jsonData
  jsonWrite(jsonData, "ssHaus", ssure);                // отправить освещеность в jsonData
  jsonWrite(jsonData, "phDHT", phdht);
